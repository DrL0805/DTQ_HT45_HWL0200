...
//		errorchk( M24SR_Deselect () );	//by lj，M24SR的I2C控制等使用完后再释放，否则若处于RF场强内，会出错
		return SUCCESS;
	}
	else
//		errorchk( M24SR_Deselect () );	//by lj，M24SR的I2C控制等使用完后再释放，否则若处于RF场强内，会出错
...
void M24SR_GetUID( uint8_t* UIDBuf)
{
	...
	status = M24SR_SelectApplication();
	status = M24SR_SelectSystemfile();
	status = M24SR_ReadBinary ( 0x0008, 7, UIDBuf );
	
//	status = M24SR_Deselect();
	
	status = status;	//防止编译器报警告
}
